...
function assertAuthorized(req: VercelRequest) {
  ...
  const xKey = getHeader(req, "x-mcp-key");
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { authToken } = assertAuthorized(req);
    const rate = assertRateLimited(req, authToken);
...
// --- Helper tool: expose available UnoCSS color tokens (read from template uno.config.ts on GitHub)
...
server.tool(
  ...
  async ({ ref }: { ref: string }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ ref, colorTokens }, null, 2),
        },
      ],
    };
  }
  ...
  // --- Helper tool: inspect base template `templates/vue-starter-template`
...
// --- Helper tool: inspect all monorepo packages under /packages
...
// --- Helper tool: describe a specific template (base or extended)
...
// --- Helper tool: compare base vs extended template (what extended adds/changes)
...
// --- Planner tool: produce an execution plan (no side-effects)
...
server.tool(
  ...
  async (input: PlanStorefrontInput) => {
    // allowlist enforcement for planned execution too (keeps model honest)
    ...
    const steps = [
      ...
      },
      {
        ...
        details: [
          ...
          "Server will scaffold project, push to GitHub and deploy to Vercel (depending on implementation in createStoreAndDeploy).",
        ],
        toolCall: {
